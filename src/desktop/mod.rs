...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let panel_bg = Color32::from_rgba_premultiplied(0, 4, 12, alpha);
        let _dim_bg = if self.sphere_selected {
            Color32::from_rgba_premultiplied(0, 4, 12, (alpha as f32 * 0.15) as u8)
        } else {
            panel_bg
        };
        ...
        // ── TRANSPARENT TOP BAR ──────────────────────────────────────────────
        let _top_bar = egui::TopBottomPanel::top("top")
            .frame(egui::Frame::none().fill(Color32::TRANSPARENT))
            .show(ctx, |ui| {
                // Drag: clicking and dragging anywhere on the top bar moves the window
        ...
        egui::CentralPanel::default().frame(egui::Frame::none().fill(Color32::TRANSPARENT)).show(ctx, |ui| {
            ...
            match self.view {
                View::Hud => {
                    let fade = if self.sphere_selected { 0.15 } else { 1.0 };
                    if !self.sphere_selected {
                        ...
                        // Emotion label fades out over 2s after emotion change
                ...
                View::Trading => {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.label(RichText::new(format!("Free: ${:.2}", free_margin)).color(Color32::from_rgb(0, 200, 255)).size(9.0).monospace());
                             if btn(ui, RichText::new(format!("{} • 1:30 ⚙", self.broker_name)).color(Color32::from_rgb(130, 100, 200)).size(9.0).monospace()).clicked() {
                                 self.settings_tab = 8; // switch to BROKERS tab in settings
                                 self.sidebar_open = true;
                             }
                             ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    // Trading-active HUD sphere (positioned below top border, above account info)
                    ...
                    // Chart header: timeframe selector + symbol label
                    ...
                    // Generate price data based on current symbol
                    ...
                    // Current price label and timeframe label
            ...
            if self.sidebar_open {
                // Full-screen semi-transparent overlay (80% transparent -> sphere visible behind)
                ...
                // Centered menu panel (larger, 80% opacity relative to 255)
                let menu_w = 440.0;
                ...
                let close_btn = Rect::from_min_size(Pos2::new(menu_rect.right() - 36.0, menu_rect.top() + 8.0), Vec2::new(28.0, 28.0));
                                    ...
                                    5 => {
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            if btn(ui, RichText::new("APPLY").size(9.0)).clicked() {
                                                // validates on next draw via primary_color()
                                            }
                                        });
...
} // end impl eframe::App for App
impl App {
    fn load_logo_texture(&mut self, ctx: &egui::Context) {
        if self.logo_texture.is_some() { return; }
        if let Ok(home) = std::env::var("HOME") {
            let path = format!("{}/.local/share/icons/hicolor/128x128/apps/umbra.png", home);
